...
### Tests
- `npx lerna run --scope @convector/convector-example-token --stream test` - Runs the unit test (simulated chaincode) `*.spec.ts`
- `npx lerna run --scope @convector/convector-example-token --stream test:e2e` - Runs the e2e test (needs the dev-env up) `*.e2e.ts`
